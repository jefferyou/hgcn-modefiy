...
def extract_osm_network_inductive():
    ...
    for poi in places:
        ...
        try:
            ...
            nx.set_edge_attributes(sub_G, poi, 'city')  # 格式：(图, 属性值, 属性名)
            print(nx.info(sub_G), '\n')
    ...
    for poi in test_places:
        ...
        try:
            ...
            # 转换为NetworkX无向图
            sub_G = nx.Graph(sub_G.to_undirected())
            nx.set_edge_attributes(sub_G, poi, 'city')
            print(nx.info(sub_G), '\n')
...
def main():
    ...
    save_original_graph(G, INDUCTIVE_DIR, PARAMS['prefix'])
    #生成拓扑邻接对
    ...
    print("You can load this data_process using the same loading mechanism as in the original GAIN code.")
    print("Example usage:")
