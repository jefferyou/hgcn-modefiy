...
"""
This script extracts road network data_process for Venice, Italy, converts it to a line graph 
...
def extract_osm_network():
    ...
    # Retrieve osm data_process by center coordinate and spatial buffer
...
def one_hot_encode_maxspeeds(g, verbose=1):
    ...
    if verbose > 0:
        print('- One-hot encoder fitted to data_process with following categories:')
...
def save_data(L, path, prefix):
    """Save processed graph data_process to files"""
...
def main():
    ...
    print("Extracting Venice road network data_process...")
    ...
    # Save the processed data_process
    if save_graphs:
