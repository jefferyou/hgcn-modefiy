...
class SHGNN_DRSD_with_HyperConv(nn.Module):
    ...
    def __init__(self, g, in_dim, out_dim, pool_dim, num_sect, rotation, num_ring,
        ...
        # First hyperbolic layer - EXPAND dimension from in_dim to in_dim*2
        self.hyp_conv_layers.append(
            HyperbolicGraphConvolution(
                self.manifold,
                in_dim,  # Input dimension
                in_dim * 2,  # Expand to double dimension
        ...
        for i in range(1, hyp_layers):
            self.hyp_conv_layers.append(
                HyperbolicGraphConvolution(
                    self.manifold,
                    in_dim * 2,  # Input is now doubled
                    in_dim * 2,  # Keep the expanded dimension
                    c_in=self.c,
                    c_out=self.c,
                    dropout=drop_rate,
                    act=F.relu,
        ...
        # SHGNN layer - now receives expanded features
        self.shgnn_layer = SHGNN_Layer(
            g, in_dim * 2, out_dim, pool_dim, num_sect, rotation, num_ring,
...
def main(args):
    ...
    # Load data
    print("Loading data...")
    data_dir = os.path.join(args.data_dir, 'DRSD')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--c', type=float, default=0.5, help='Hyperbolic curvature')
